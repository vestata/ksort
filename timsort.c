...
static int timsort_init(struct timsort *ts,
                        void *a,
                        size_t len,
                        CMPPARAMS(c, carg),
                        size_t width);
...
static void *ensureCapacity(struct timsort *ts,
                            size_t minCapacity,
                            size_t width);
                        ...
                        size_t width)
{
    ...
    if (ts->tmp_length) {
        ts->tmp = kmalloc(ts->tmp_length * width, GFP_KERNEL);
        err |= ts->tmp == NULL;
    } else {
        ts->tmp = NULL;
    }
    ...
    ts->run = kmalloc(ts->stackLen * sizeof(ts->run[0]));
    err |= ts->run == NULL;
#else
